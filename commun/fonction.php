...
/**
 * Cette fonction permet de verifier si un utilisateur dans la base de donnée ne possède pas cette email mail.
...
function check_password_exist(string $mot_passe, int $id){
    $users="false";
    $db= connect_db();
    $req= $db->prepare('SELECT id from utilisateur WHERE mot_passe=:mot_passe AND id=:id');
    $req->execute(array(
        'mot_passe'=>sha1($mot_passe),
        'id'=>$id
    ));
...
function update_profil_in_db( int $id,  string $nom,  string $prenom,  string $email, string $nom_utilisateur,string $profile ,string $mot_passe  ){
    $modifier_profil="false";
    $db= connect_db();
    $requete= "UPDATE utilisateur SET nom= :nom, prenom=:prenom, email= :email, nom_utilisateur= :nom_utilisateur ,
     profile = :profile,  mot_passe= :mot_passe  WHERE id_utilisateur = :id";
...
function recup_udpate_profil($id){
    $donnees=[];
    $db= connect_db();
    $req_recup=$db->prepare('SELECT id, nom, prenom, nom_utilisateur, profile, email   FROM utilisateur WHERE id_utilisateur= :id');
    $resultat=$req_recup->execute(array(
       'id'=> $id,
    ));

    if($resultat){
        $donnees=$req_recup->fetchAll(PDO::FETCH_ASSOC);
    }
    return $donnees;
}